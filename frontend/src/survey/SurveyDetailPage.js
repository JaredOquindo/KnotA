...
export default function SurveyDetailPage() {
  ...
  useEffect(() => {
    const fetchSurvey = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/surveys/${id}`);
    ...
    const fetchSubmissions = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/surveys/${id}/responses`);
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this survey?")) {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/surveys/${id}`, { method: "DELETE" });
  ...
  const handleClose = async () => {
    if (window.confirm("Are you sure you want to close this survey?")) {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/surveys/${id}/close`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
        });
  ...
  const handleDeleteSubmission = async (submissionId) => {
    if (window.confirm("Are you sure you want to delete this submission?")) {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/surveys/${id}/responses/${submissionId}`, {
          method: "DELETE",
        });
