...
export default function PendingPageAdmin() {
  ...
  const fetchPendings = useCallback(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/institutions/pending?${params.toString()}`)
      .then((res) => {
  ...
  const approveInstitution = async (id) => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/institutions/${id}/approve`, { method: "PATCH" });
  ...
  const deleteInstitution = async (id) => {
    ...
    try {
      await fetch(`${process.env.REACT_APP_API_URL}/institutions/${id}`, { method: "DELETE" });
      fetchPendings();
    } catch (err) {
      console.error(err);
